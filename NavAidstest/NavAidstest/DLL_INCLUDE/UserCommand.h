...
namespace NS_SALUS_MS_APP
{
	...
	private:
		BOOL IsExistCommand(E_USER_COMMAND eCommand);	// 맵에 해당 command의 존재 여부 확인
		void AddCommand(E_USER_COMMAND eCommand);		// 맵에 해당 command 추가
	...
	private:
		...
		std::queue<std::pair<E_USER_COMMAND, UINT>> m_queueCommand;	// 명령 종류와 버퍼 인덱스 저장
		std::map<E_USER_COMMAND, UINT> m_mapCommand;				// 명령 종류와 명령 발생 횟수 저장
