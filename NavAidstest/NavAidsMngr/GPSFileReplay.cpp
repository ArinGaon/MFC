...
namespace NS_SALUS_MS_APP
{
	CGPSFileReplay::CGPSFileReplay()
	{
		...
		m_hFileReplayExitEvent = CreateEvent(NULL, FALSE, FALSE, _T("GPS FILE REPLAY EXIT EVENT"));
		m_hFileReplayWorkEvent = CreateEvent(NULL, FALSE, FALSE, _T("GPS FILE REPLAY WORK EVENT"));
	...
	void CGPSFileReplay::SetMsgManager(CSerialMsgMngr* const pSerialMsgMngr)
	{
		m_pSerialMsgMngr = pSerialMsgMngr;
	}

	void CGPSFileReplay::StartReplay(const TCHAR* const pszReplayFileName)
	{
		if (m_pDataReader != NULL)
		{
	...
	void CGPSFileReplay::StopRepaly()
	{
		ExitFileReplayThread();
	}

	HANDLE CGPSFileReplay::GetReplayWorkEvent()
	{
		return m_hFileReplayWorkEvent;
	}

	void CGPSFileReplay::CreateFileReplayThread()
	{
	...
	void CGPSFileReplay::ExitFileReplayThread()
	{
		if (m_pFileReplayThread != NULL)
		{
	...
	void CGPSFileReplay::FileReplayThread()
	{
	...
	UINT CGPSFileReplay::FileReplayThread(LPVOID const lpParam)
	{
		CGPSFileReplay* const pFileReplay = static_cast<CGPSFileReplay*>(lpParam);
